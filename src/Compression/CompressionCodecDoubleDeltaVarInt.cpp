 ...
 * which encodes as a single byte (0x3F = 63 + 0 = zero delta-of-delta).
 */
...
{
    setCodecDescription("DoubleDeltaVarInt", {std::make_shared<ASTLiteral>(static_cast<UInt64>(data_bytes_size))});
}
...
void CompressionCodecDoubleDeltaVarInt::doDecompressData(const char * source, UInt32 source_size, char * dest, UInt32 uncompressed_size) const
{
    if (source_size < 2)
